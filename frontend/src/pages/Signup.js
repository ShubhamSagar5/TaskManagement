...
const handleSignUp = async() => {
  try {
    ...
    const res = await axios.post('https://taskmanagement-tavq.onrender.com/api/v1/signup',data)
    if(res?.data?.success){
...
useEffect(()=>{
  ...
  return (
    <div className='h-[100vh] md:h-[98%] flex justify-center items-center'>
        ...
        <div className='flex bg-gray- flex-col md:w-3/12 p-1 rounded-lg border border-gray-200'>
            ...
            <div className='flex flex-col items-center'>
                 <button className='bg-blue-900 w-10/12 ml-[18px] md:ml-0 p-1  rounded-md m-3' onClick={handleSignUp}>Signup</button>
            <Link to={"/login"}><p className='p-2 md:p-0'>Already Account ? Please Login</p></Link>
            </div>
